...
{
    minPt.x = fmin(minPt.x, vec->x);
    minPt.y = fmin(minPt.y, vec->y);
    minPt.z = fmin(minPt.z, vec->z);
    
    maxPt.x = fmax(maxPt.x, vec->x);
    maxPt.y = fmax(maxPt.y, vec->y);
    maxPt.z = fmax(maxPt.z, vec->z);
}

void XBoundingBox::merge(const XBoundingBox* bbox)
{
    minPt.x = fmin(minPt.x, bbox->minPt.x);
    minPt.y = fmin(minPt.y, bbox->minPt.y);
    minPt.z = fmin(minPt.z, bbox->minPt.z);
    
    maxPt.x = fmax(maxPt.x, bbox->maxPt.x);
    maxPt.y = fmax(maxPt.y, bbox->maxPt.y);
    maxPt.z = fmax(maxPt.z, bbox->maxPt.z);
}
