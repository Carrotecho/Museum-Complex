...
void ShaderCache::getShadersName(Material*mat ,Mesh*model, uint pass,vector<string> *outList){
    if (!model) return;
    ...
    switch (pass) {
        case SHADOW_DEPTH_TEHNICS:
            // XTODO : mb use array ?
            if (mat->getFlagVal(MATERIAL_OBJECT_POSITION))         outList->push_back("ObjectPosition.xml");
        ...
        case BACK_BUFFER_TEHNICS:
            if (mat->getFlagVal(BASE_MATERIAL_COLOR))                  outList->push_back("baseMaterial.xml");
            if (mat->getFlagVal(NORMAL_MATERIAL))                      outList->push_back("normalMaterial.xml");
            if (mat->getFlagVal(COLOR_MATERIAL))                       outList->push_back("colorMaterial.xml");
            if (mat->getFlagVal(SPECULAR_MATERIAL))                    outList->push_back("specularMapMaterial.xml");
            if (mat->getFlagVal(NORMAL_MAP_MATERIAL))                  outList->push_back("normalMapMaterial.xml");
            if (mat->getFlagVal(TEXTURE_AND_COLOR))                    outList->push_back("textureAndColor.xml");
            
            
            if (mat->getFlagVal(DYNAMIC_PER_VERTEX_BASE_LIGHT))    outList->push_back("baseDynamicPerVertexLight.xml");
            if (mat->getFlagVal(DYNAMIC_PER_PIXEL_BASE_LIGHT))     outList->push_back("baseDynamicPerPixelLight.xml");
            if (mat->getFlagVal(RESAVE_SHADOW_SM))                 outList->push_back("ShadowMapsBackBuffer.xml");
            if (mat->getFlagVal(MATERIAL_COLOR_FROM_MAT))          outList->push_back("colorFromMaterial.xml");
            if (mat->getFlagVal(PARTICLE_MATERIAL))                outList->push_back("particles.xml");
            if (mat->getFlagVal(MATERIAL_PROJECT))                 outList->push_back("projection.xml");
            if (mat->getFlagVal(MATERIAL_DISCARD_Y))               outList->push_back("cutY.xml");
            if (mat->getFlagVal(MATERIAL_OBJECT_POSITION))         outList->push_back("ObjectPosition.xml");
            ...
            break;
        case PICK_OBJECT_TEHNICS:
            outList->push_back("pickingObject.xml");
...
ShaderCache::ShaderCache(std::string _resourcePath){
    ...
    shaderDeclarationTableURL = resourcePath;
    shaderDeclarationTableURL+= "shaderDeclarationTable.xml";
    
    
    XLOG("shaderDeclarationTableURL:%s",shaderDeclarationTableURL.c_str());
    ...
    action[UNIFORM_INDEX_MESH]                      = &ShaderParamsSetter::setObjectIndex;
    action[UNIFORM_COLOR_MATERIAL]                  = &ShaderParamsSetter::setMaterialColor;
    action[UNIFORM_CAMERA_RIGHT_WS]                 = &ShaderParamsSetter::setCameraRightWS;
    action[UNIFORM_CAMERA_UP_WS]                    = &ShaderParamsSetter::setCameraUpWS;
    action[UNIFORM_SPECULAR_POWER]                  = &ShaderParamsSetter::setSpecularPower;
...
Shader* ShaderCache::get(Material*mat ,Mesh*model, uint pass){
    ...
    std::string matFlags = mat->matFlagsString;
    
    
    uint udidint = (mat->shader_id) |  model->vertexDeclaration << 8 | pass << 16;
    char str[16];
    itostr(udidint, str);
    uid = str;
    uid+=matFlags;
...
ShaderCache::~ShaderCache(){
    ...
    std::map<std::string, Shader*>::iterator it = shadersCache.begin();
    while ( it != shadersCache.end() ) {
        Shader*shader = it->second;
        shader->deleteShader();
        free(shader);
        it = shadersCache.erase(it);
    }
...
Shader* ShaderCache::createShader(Material*mat ,Mesh*model, uint pass){
    ...
    getShadersName(mat,model,pass,&shaderList);
    
    
    for (int i = 0; i < shaderList.size(); i++) {
        XLOG("shaderName %s",shaderList[i].c_str());
    }
    
    
    char*shadersPath = (char*)resourcePath.c_str();
    
    
    XLOG("shaderpath%s",shadersPath);
    ...
    XLOG("%s",vertexShaderSource.c_str());
    XLOG("%s",pixelShaderSource.c_str());
...
Shader* ShaderCache::createShaderProgram(const char*vertexSource,const char*pixelSource,Mesh*model){
	...
	if (logLength > 0) 
	{
		...
		XLOG("Vtx Shader compile log:%s\n", log);
		free(log);
	}
    glGetShaderiv(vShaderName, GL_COMPILE_STATUS, &status);
	if (status == 0)
	{
		XLOG("Failed to compile vtx shader:\n%s\n",vertexSource);
		return 0;
	}
	...
	if (status == 0)
	{
		XLOG("Failed to compile frag shader:\n%s\n", pixelSource);
		return 0;
	}
	...
	if (logLength > 0)
	{
		...
		XLOG("Program link log:\n%s\n", log);
		free(log);
	}
    glGetProgramiv(program, GL_LINK_STATUS, &status);
	if (status == 0)
	{
		XLOG("Failed to link program");
		return 0;
	}
	...
	if (status == 0)
	{
		XLOG("Failed to validate program");
		//return 0;
	}    
...
void ShaderCache::fillExtensions(pugi::xml_node *shader){
    ...
    if (!xml.empty()) {
        for (pugi::xml_node extension = xml.child("extension"); extension; extension = extension.next_sibling("extension"))
        {
