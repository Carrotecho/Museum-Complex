...
void ShaderParamsSetter::setCameraRightWS(uint idUniform) {
    glUniform3f(idUniform,camera->getModelViewMatrix()->m00,camera->getModelViewMatrix()->m10,camera->getModelViewMatrix()->m20);
}

//
void ShaderParamsSetter::setSpecularPower(uint idUniform) {
    Material*mat = baseRenderObject->getMaterial();
    glUniform1f(idUniform,mat->getSpecularPower());
}

void ShaderParamsSetter::setCameraUpWS(uint idUniform) {
    glUniform3f(idUniform,camera->getModelViewMatrix()->m01,camera->getModelViewMatrix()->m11,camera->getModelViewMatrix()->m21);
}

void ShaderParamsSetter::setMaterialColor(uint idUniform) {
    Material*mat = baseRenderObject->getMaterial();
    glUniform3f(idUniform,mat->color.x,mat->color.y,mat->color.z);
}

void ShaderParamsSetter::setObjectIndex(uint idUniform) {
    glUniform1f(idUniform,baseRenderObject->indexMesh);
}
...
void ShaderParamsSetter::setLightPositionUniform(uint idUniform){
	...
	glUniform3f(idUniform, light->getPosition()->x,light->getPosition()->y,light->getPosition()->z);
}
