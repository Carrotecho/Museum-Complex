...
#endif
void MessageBox( void *ignored1, string message, string title, int ignored2 )
{ 
    printf("error title:%s message:%s",message.c_str(),title.c_str());
}
GLubyte uTGAcompare[12]= {0,0,2, 0,0,0,0,0,0,0,0,0};
GLubyte cTGAcompare[12]= {0,0,10,0,0,0,0,0,0,0,0,0};

bool TGAImage::LoadTGA(Texture * texture, const char * filename)				// Load a TGA file
{
...
bool TGAImage::LoadUncompressedTGA(Texture * texture, const char * filename, FILE * fTGA)	// Load an uncompressed TGA (note, much of this code is based on NeHe's 
{																			// TGA Loading code nehe.gamedev.net)
	...
	{
		red = texture->imageData[cswap];
		texture->imageData[cswap] = texture->imageData[cswap+2];
		texture->imageData[cswap+2] = red;
	}
    fclose(fTGA);															// Close file
	return true;															// Return success
}

bool TGAImage::LoadCompressedTGA(Texture * texture, const char * filename, FILE * fTGA)		// Load COMPRESSED TGAs
{ 
