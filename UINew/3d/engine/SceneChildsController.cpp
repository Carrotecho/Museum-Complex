...
XQuery* SceneChildsController::getNewQuery() {
    ...
    if (numQuerys+1>=sizeAlloced) {
        ...
        for (unsigned int i = numQuerys; i<sizeAlloced; i++) {
			...
			querys[i].renderID = FORWARD_RENDERER;
            querys[i].size = 0;
        }
    }
...
{
	inline bool operator() (const SceneNode* struct1, const SceneNode* struct2)
	{
		return (struct1->zTest > struct2->zTest);
	}
};
struct less_than_key
{
	inline bool operator() (const SceneNode* struct1, const SceneNode* struct2)
	{
		return (struct1->blendFunction < struct2->blendFunction);
	}
};
#include <algorithm>
void SceneChildsController::createQueryList() {
	...
	sort(childsList.begin(), childsList.end(), less_than_key());
	//sort(childsList.begin(), childsList.end(), less_than_key2());
